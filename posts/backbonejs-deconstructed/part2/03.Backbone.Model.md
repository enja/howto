...
- モデルのセットアップ時に使われる`initialize()`関数についても忘れてはならない。
...
テンプレートのために、クリーンなコピーを得たい時に楽な方法である。
...
モデルの`validate`関数が失敗すると、ビルドインの`invalid`イベントが伝播する。コード例では、このイベントを監視し、渡されたパラメータを出力するコールバック関数を指定している。
