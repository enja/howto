...
Backboneのコレクションはモデルの論理的なグルーピングを提供し、それらのモデルに対して作用する（グルーピングや、ソート、フィルタ等）関数やプロパティを提供する。
この記事で使ってきた、連絡先を使った表現（モデルは連絡先一覧のラベル付けされた行データのような）を用いると、コレクションは連絡先のデータ行を含む表全体と捉えることが出来るだろう。
モデルのセクションで議論してきた連絡先の表を更新し、表全体を連絡先一覧と捉えられるようにした。
...
Of course, a `Backbone.Collection` is much more than a label like the previous table might express.
Collections can capture events (change, destroy, request, sync, error, invalid) that are triggered on its models and collections also have their own set of useful events (e.g. add, remove, reset, sort, request, sync).

もちろん、`Backbone.Collection`は前で述べたテーブルが表現するラベルのようなものだ。
コレクションは
